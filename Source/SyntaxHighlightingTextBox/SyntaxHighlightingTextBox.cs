...
namespace UrielGuy.SyntaxHighlightingTextBox
{
    ...
    public class SyntaxHighlightingTextBox : System.Windows.Forms.RichTextBox
    {
                ...
                for (int i = 0; i < line.Length; )
                {
                    ...
                    if (mSeperators.Contains(curChar))
                    {
                        if (curChar == '\"')
                        {
                            ...
                            SetDescriptorSettings(sb, HighlightDescriptors[0], colors, fonts);
                    ...
                    {
                        bool isInString = stringCount % 2 != 0;
                                ...
                                case Keys.Down:
                                    {
                                        ...
                                        mAutoCompleteForm.List.EnsureVisible(mAutoCompleteForm.SelectedIndex);
                                        return;
                                    }
                                case Keys.Up:
                                    {
        ...
        {
            string curTokenString = string.Empty;

            if (!string.IsNullOrEmpty(Text))
            {
                int curTokenStartIndex = Text.LastIndexOfAny(mSeperators.GetAsCharArray(), Math.Max(SelectionStart - 1, 0)) + 1;
                int curTokenEndIndex = Text.IndexOfAny(mSeperators.GetAsCharArray(), Math.Min(SelectionStart, Text.Length - 1));
               ...
               curTokenString = Text.Substring(curTokenStartIndex, Math.Max(curTokenEndIndex - curTokenStartIndex, 0)).ToUpper();
            }
           
            ShowAutoComplete(curTokenString);
        ...
        public bool AcceptAutoCompleteItem()
        {
            ...
            int curTokenStartIndex = 0;
            int curTokenEndIndex = 0;

            if (!string.IsNullOrEmpty(Text))
            {
                curTokenStartIndex = Text.LastIndexOfAny(mSeperators.GetAsCharArray(), Math.Max(SelectionStart - 1, 0)) + 1;
                curTokenEndIndex = Text.IndexOfAny(mSeperators.GetAsCharArray(), Math.Min(SelectionStart, Text.Length - 1));
        ...
        private void SetAutoCompleteItems()
        {
            ...
            foreach (HighlightDescriptor hd in mHighlightDescriptors)
            {
                ...
                {
                    items.Add(hd.Token[0].ToString().ToUpper() + hd.Token.Substring(1,hd.Token.Length -1).ToLower());
                }
            }

            ArrayList array = new ArrayList(items);
            array.Sort(new CaseInsensitiveComparer());

            mAutoCompleteForm.Items.AddRange((string[])array.ToArray(typeof(string)));
        ...
        public void ShowAutoComplete(string token)
        {
            SetAutoCompleteItems();
            SetAutoCompleteSize();
            SetAutoCompleteLocation(true);
            ...
            foreach (var keyword in mAutoCompleteForm.Items)
            {
                if (keyword.ToUpper().Contains(token))
                {
                    mAutoCompleteForm.SelectedIndex = index;
                    mAutoCompleteForm.Select();
                    mAutoCompleteForm.List.EnsureVisible(index);
        ...
        private void AddFontToTable(StringBuilder sb, Font font, ref int counter, Hashtable fonts)
        {
            ...
            if (fonts.Contains(font.Name)) fonts.Remove(font.Name);
